...
//    }
    ...
    private void init() {
        ...
        if (datum.size() > 0) {
            ...
            if (StringUtils.isNotBlank(id)) {
                //在播放列表中查看该歌曲
                KnobbleDetailsInfoVo song = null;
                for (KnobbleDetailsInfoVo s : datum) {
    ...
    @Override
    public List<KnobbleDetailsInfoVo> getPlayList() {
        return datum;
    }
    ...
    @Override
    public void play(KnobbleDetailsInfoVo song) {
        isPlay = true;
...
//
//        }
        musicPlayer.play(song.getMp3Url(), song);
...
//            }
    ...
    @Override
    public KnobbleDetailsInfoVo next() {
        if (datum.size() == 0) {
            return null;
        }
        switch (model) {
            case MODEL_LOOP_RANDOM:
                ...
                KnobbleDetailsInfoVo song = datum.get(i);
                return song;
            default:
    ...
    @Override
    public KnobbleDetailsInfoVo previous() {
        if (datum.size() == 0) {
            return null;
        }
        switch (model) {
...
//        floatingLayoutManager.update(currentSong);
    ...
    @Override
    public void nextPlay(KnobbleDetailsInfoVo song) {
        int index = datum.indexOf(currentSong);
        if (index != -1) {
            //先移除原来的歌曲，可能存在于列表
            datum.remove(song);
